...
1. Вы — Наблюдатель
    ...
    В случае, если пользователь не может присоединиться к какой-либо команде, он может наблюдать за ходом чьей-либо чужой игры. В этом случае будем называть его *Наблюдателем* (viewer). Сервер может превратить наблюдателя в игрока в любой момент, когда наберётся два наблюдателя.
...
1. Вы — игрок
    ...
    В случае, если пользователя «только и дожидался» один человек, формируется новая команда, участником которой он немедленно становится (команды формируются из пар пользователей).
...
1. Пользователь подключился
    ...
    В случае, если на сайт зашёл ещё один Наблюдатель, но пока общее количество Наблюдателей меньше двух, сервер посылает клиенту это сообщение.
...
1. Новое состояние подсказки Художника
    ...
    Эти сообщения получают Кассандра и Наблюдатели — все, кроме Художника, по мере того, как Художник вводит подсказку Кассандре.
...
1. Кассандра приняла решение
    ...
    Кассандра высказывает предположение.
